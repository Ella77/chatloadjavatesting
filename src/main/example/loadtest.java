...
public class Example extends WebSocketClient {

	static int received = 0;
	static int send = 0;
	static long sum = 0;

	public Example(URI serverUri, Draft draft) {
		super(serverUri, draft);
	}

	public Example(URI serverURI) {
		super(serverURI);
	}

	public Example(URI serverUri, Map<String, String> httpHeaders) {
	...
	private static boolean allConnected(List<Example> connections) {
		int count = 0;
	...
	static class Sending implements Runnable {
		private Example e;
		public Sending(Example example) {
			this.e = example;
		}
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i = 0; i < clients; i++) {
			Socket sc = IO.socket("http://52.79.243.195:3000");
			sc.connect();
			...
			System.out.println(sc.id() + " has connected");
